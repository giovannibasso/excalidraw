...
export const replaceModeAtom = atom<boolean>(false);
const replaceValueAtom = atom<string>("");
...
export const SearchMenu = () => {
  ...
  const [replaceValue, setReplaceValue] = useAtom(replaceValueAtom);
  ...
  }, [setAppState, setFocusIndex, setReplaceMode, setReplaceValue]);

  const replaceMatch = () => {
    if (focusIndex === null || !searchMatches.items[focusIndex]) {
      return;
    }
    const match = searchMatches.items[focusIndex];
    ...
    if (isTextElement(element)) {
      ...
      const newText =
        ...
        replaceValue +
        originalText.slice(match.index + searchQuery.length);
      ...
      const container = app.scene.getContainerElement(element);
      redrawTextBoundingBox(element, container, app.scene);
    } else if (isFrameLikeElement(element)) {
      const name = element.name ?? getDefaultFrameName(element);
      const newName =
        name.slice(0, match.index) +
        replaceValue +
        name.slice(match.index + searchQuery.length);
    ...
    // Force search refresh by invalidating the nonce
    lastSceneNonceRef.current = undefined;
    searchedQueryRef.current = null;
  };
  ...
  const replaceAll = () => {
    ...
    // Group matches by element and process in reverse index order
    const matchesByElement = new Map<
      string,
      SearchMatchItem[]
    >();
    for (const match of searchMatches.items) {
      const id = match.element.id;
      if (!matchesByElement.has(id)) {
        matchesByElement.set(id, []);
      }
      matchesByElement.get(id)!.push(match);
    }

    for (const [, matches] of matchesByElement) {
      ...
      if (isTextElement(element)) {
        ...
        for (const match of sorted) {
          text =
            ...
            replaceValue +
            text.slice(match.index + searchQuery.length);
        }
        app.scene.mutateElement(element, {
          originalText: text,
          text,
        ...
        let name = element.name ?? getDefaultFrameName(element);
        for (const match of sorted) {
          name =
            name.slice(0, match.index) +
            replaceValue +
  ...
  useEffect(() => {
    const eventHandler = (event: KeyboardEvent) => {
      ...
      if (event[KEYS.CTRL_OR_CMD] && event.key === KEYS.H) {
        event.preventDefault();
        event.stopPropagation();
  ...
  }, [setAppState, stableState, app, setReplaceMode]);
    ...
    <div className="layer-ui__search">
      ...
      {replaceMode && (
        <div className="layer-ui__replace-header">
          <TextField
            className="layer-ui__replace-input-wrapper"
            value={replaceValue}
            ref={replaceInputRef}
          ...
          />
          <div className="layer-ui__replace-actions">
            <Button
              onSelect={replaceMatch}
              className="replace-btn"
              disabled={
                focusIndex === null || searchMatches.items.length === 0
              }
            >
              {t("search.replace")}
            </Button>
            <Button
              onSelect={replaceAll}
              className="replace-btn"
              disabled={searchMatches.items.length === 0}
            >
              {t("search.replaceAll")}
            </Button>
          </div>
