...
>({
  ...
  trackEvent: false,
  perform: (elements, appState, value) => {
    if (typeof value === "object" && value !== null && "radius" in value) {
      const radius = Math.max(0, Math.min(128, value.radius));
      return {
        elements: changeProperty(elements, appState, (el) => {
          if (isElbowArrow(el)) {
            return el;
  ...
  PanelComponent: ({ elements, appState, updateData, app, renderAction }) => {
    ...
    const showRadiusInput =
      ...
      targetElements.some(
        (el) => isUsingAdaptiveRadius(el.type) && el.roundness,
      );

    const currentRadius = showRadiusInput
      ? reduceToCommonValue(
          targetElements.filter(
            (el) => isUsingAdaptiveRadius(el.type) && el.roundness,
          ),
          (el) => el.roundness?.value ?? 32,
        ) ?? 32
      : 32;
          ...
          >
            ...
            <input
              ...
              onBlur={(e) => {
                ...
                if (!isNaN(val)) {
                  updateData({
                    radius: Math.max(0, Math.min(128, val)),
                  });
                }
              }}
              style={{
            ...
            />
            <span style={{ fontSize: "0.75rem", color: "var(--color-gray-60)" }}>
              px
            </span>
          </div>
        )}
